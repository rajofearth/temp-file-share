...
Both `outbound_domain_allowlist` and `outbound_cidr_allowlist` can be passed to
`_experimental_set_outbound_network_policy` and work the same way as on
`Sandbox.create`.
...
* Only available in the Python SDK. Go and JavaScript support is not yet
  available.
